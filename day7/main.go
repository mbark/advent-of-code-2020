...
type BagDef struct {
	Bag      Bag
	Contains map[string]ContainedBag
}
...
func main() {
	...
	for _, s := range split {
		...
		contains := make(map[string]ContainedBag)
		for _, d := range strings.Split(spl[1], ",") {
			// 1 bright white bag
			f := strings.Split(strings.Trim(d, " "), " ")
			if f[0] == "no" {
				continue
			}
			...
			contains[cb.Bag.ID()] = cb
		}
		n.Contains = contains
		bags[b.ID()] = n
	}

	startBag := Bag{Tint: "shiny", Color: "gold"}

	fmt.Printf("first %d\n", first(startBag, bags))
	fmt.Printf("second %d\n", second(startBag, bags))
}

func first(start Bag, def map[string]BagDef) int {
	foundBags := map[string]Bag{}
	prev := make(map[string]Bag)
	prev[start.ID()] = start

	for len(prev) > 0 {
		added := make(map[string]Bag)

		for _, d := range def {
			for _, c := range d.Contains {
				for _, a := range prev {
					if a == c.Bag {
						added[d.Bag.ID()] = d.Bag
					}
				}
			}
		}
...
func second(start Bag, def map[string]BagDef) int {
	var foundBags []ContainedBag
	prev := []ContainedBag{{Bag: start, Count: 1}}

	for len(prev) > 0 {
		var added []ContainedBag
		...
		for _, d := range def {
			for _, a := range prev {
				...
				for _, c := range d.Contains {
					added = append(added, ContainedBag{
						Bag:   c.Bag,
		...
		foundBags = append(foundBags, added...)
	}

	var total int
	for _, b := range foundBags {
		total += b.Count
	}
