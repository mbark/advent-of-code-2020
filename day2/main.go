...
func main() {
	r := regexp.MustCompile("(?P<Min>[0-9]+)-(?P<Max>[0-9]+) (?P<Char>[a-z]): (?P<Password>[a-z]+)")
	...
	for _, s := range input {
		...
		for i, n := range r.SubexpNames() {
			...
			case "Max":
				...
				inp.Max = j
			case "Char":
				inp.Letter = matches[i]
			case "Password":
				inp.Password = matches[i]
			}
		}
...
func first(input []Input) int {
	valid := 0
	for _, in := range input {
		count := strings.Count(in.Password, in.Letter)
...
func second(input []Input) int {
	valid := 0

	hasAt := func(in Input, i int) bool {
		i = i - 1
		if len(in.Password) <= i {
			return false
		}

		return rune(in.Password[i]) == rune(in.Letter[0])
	}

	for _, in := range input {
		hasAtFirst := hasAt(in, in.Min)
		hasAtSecond := hasAt(in, in.Max)

		if hasAtFirst && !hasAtSecond || hasAtSecond && !hasAtFirst {
			valid += 1
		}
	}
