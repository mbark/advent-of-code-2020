...
func main() {
	...
	for _, s := range split {
		passports = append(passports, strings.ReplaceAll(s, "\n", " "))
	}
...
func hasAllFields(p string) bool {
	req := []string{
		"byr",
		"iyr",
		"eyr",
	...
	allReqFields := true
	for _, f := range req {
		allReqFields = allReqFields && strings.Contains(p, f)
	}

	return allReqFields
}

func first(passports []string) int {
	valid := 0
...
func validateField(p, key, val string) bool {
	switch key {
	case "byr":
	...
	case "hcl":
		...
		re := regexp.MustCompile("([a-z]|[0-9]){6}")
		return re.MatchString(nr)

	case "ecl":
		mustBe := []string{"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}
		for _, b := range mustBe {
			if val == b {
				return true
			}
		}
...
func allFieldValuesValid(p string) bool {
	if !hasAllFields(p) {
		return false
	}
	...
	for _, f := range fields {
		...
		isValid := validateField(p, key, val)
		if !isValid {
			return false
		}
...
func second(passports []string) int {
	valid := 0
	for _, p := range passports {
		if allFieldValuesValid(p) {
			valid += 1
		}
	}
